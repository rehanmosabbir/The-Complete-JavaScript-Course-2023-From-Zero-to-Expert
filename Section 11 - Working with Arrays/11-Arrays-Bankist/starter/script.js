...
console.log('rehan'.at(2));
console.log('rehan'.at(-1));
...
/*
const dogsJuliaData = [9, 16, 6, 8, 3];
const dogsKateData = [10, 5, 6, 1, 4];
const checkDogs = function (dogsJulia, dogsKate) {
  const dogsJuliaCorrected = dogsJulia.slice();
  dogsJuliaCorrected.splice(0, 1);
  dogsJuliaCorrected.splice(-2);
  const dogsJuliaKateNew = [...dogsJuliaCorrected, ...dogsKate];
  dogsJuliaKateNew.forEach(function (age, i) {
    if (age >= 3) {
...
const calcAverageHumanAge = function (ages) {
  const humanAges = ages.map((dogAge, i, arr) => {
    if (dogAge <= 2) return dogAge * 2;
    else if (dogAge > 2) return 16 + dogAge * 4;
  });
  console.log(humanAges);
  const adultDogs = humanAges.filter((humanAge, i, arr) => {
    return humanAge >= 18;
  });
  console.log(adultDogs);
  const sumAge = adultDogs.reduce((acc, age, i, arr) => {
    return acc + age;
  }, 0);
  console.log(sumAge / adultDogs.length);
};
...
const totalMovements3 = accounts
  .flatMap(acc => acc.movements)
  .reduce((acc, mov) => acc + mov, 0);
console.log(totalMovements3);
...
const rand = Array.from({ length: 100 }, (cur, i) => {
...
const convertTitleCase = function (title) {
  ...
  const exceptions = ['a', 'an', 'the', 'and', 'or', 'but', 'on', 'in', 'with'];
